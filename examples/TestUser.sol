...
contract TestUser {
    address rngAddress; // Stochastic DAO's random number generator contract address.
    ...
    function generateNumber() public payable returns (uint) {
        (bool success, bytes memory result) = rngAddress.call(abi.encodeWithSignature("rngFee()")); // Get fee from Stochastic DAO's RNG contract.
